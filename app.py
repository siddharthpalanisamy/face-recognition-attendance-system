...
def check_existing(name, userid):
    userlist, names, rolls, _ = getallusers()
    if name in names:
...
def start():
    ...
    while ret:
        ...
        if len(extract_faces(frame)) > 0:
            ...
            break  # Break the loop after identifying the face and marking attendance
    ...
    # Extract updated attendance after marking
    names, rolls, times, l = extract_attendance()
...
def add():
    if request.method == 'POST':
        ...
        exists, error_msg = check_existing(newusername, newuserid)
        if exists:
            return render_template('home.html', mess=error_msg, totalreg=totalreg(), datetoday2=datetoday2)
